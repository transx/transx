/* *Class created on [ Jun 17, 2008 | 10:51:10 AM ] */
...
import com.asta.app2.tutorial.util.Messages;
...
public class TabbedPaneRenderer extends Renderer {
	private static Logger logger = Logger
			.getLogger("com.asta.app2.tutorial.util");

	/**
	 * By default, getRendersChildren() returns false, so encodeChildren() won't
	 * be invoked unless we override getRendersChildren() to return true
	 */
	...
	/**
	 * The decode method gets the value of the request parameter whose name is
	 * the client Id of the tabbedpane component. The request parameter is
	 * encoded as a hidden field by encodeHiddenField, which is called by
	 * encodeEnd. The value for the parameter is set by JavaScript generated by
	 * the encodeTab method. It is the name of a facet or a JSP page. The decode
	 * method uses the request parameter value to set the tabbedpane component's
	 * content attribute. Finally, decode() queues an action event that's fired
	 * to registered listeners in the Invoke Application phase of the JSF
	 * lifecycle. Action listeners can be specified with the
	 * <corejsf:tabbedpane>'s actionListener attribute or with
	 * <f:actionListener> tags in the body of the <corejsf:tabbedpane> tag.
	 */
	...
	/**
	 * The encodeBegin method writes the starting <table> HTML element with the
	 * CSS class specified by the <corejsf:tabbedpane>'s styleClass attribute
	 * (if supplied)
	 */
	@Override
	 ...
	 * encodeBegin(). The children of the <corejsf:tabbedpane> component are
	 * UISelectItem components, set with one or more <f:selectItem> tags or a
	 * single <f:selectItems> tag in the body of <corejsf:tabbedpane>
	 */
	@Override
	 ...
	 * encodeEnd() writes the table body and encodes the tabbedpane's content in
	 * a single table row. The content for the tabbed pane can be specified as
	 * either a URL for a JSP page or a facet name, so encodeEnd() checks to see
	 * if it's a facet; if so, it encodes it; if not, it includes the JSP page
	 */
	@Override
	...
	/**
	 * The encodeHiddenField method is called at the end of encodeEnd(). See the
	 * decode method for an explanation of the field and its value.
	 * 
	 * @param context
	 * @param writer
	 * @param component
	 ...
	 * encodeTab also writes out a class attribute for each tab corresponding to
	 * either the tabClass attribute (for unselected tabs) or the
	...
	/**
	 * Text for the tabs in the tabbedpane component can be specified as a key
	 * in a resource bundle, or as the actual text that's displayed in the tab.
	 * Given that text, the getLocalizedTabText method tries to retrieve a value
	 * from the resource bundle specified with the <corejsf:tabbedpane>'s
	 * resourceBundle attribute. If no value is found, getLocalizedTabText just
	 * returns the string it was passed.
	 * 
	 * @param component
	 * @param label
	 * @return
